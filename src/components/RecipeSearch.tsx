...
export default function RecipeSearch({ cards, searchPlaceholder, noResults, minutes, prep, total, recipeHeading }: Props) {
  ...
  return (
    <div>
      <div className="mt-8 grid grid-cols-[repeat(auto-fit,minmax(min(420px,100%),1fr))] items-end gap-4">
        <h2 className="m-0 font-heading text-xl text-ink">{recipeHeading}</h2>
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full rounded-lg border-[1.5px] border-ink bg-surface py-2 pl-10 pr-3 text-sm"
          />
        </div>
      </div>
        ...
        <div className="mt-3 grid grid-cols-[repeat(auto-fit,minmax(min(420px,100%),1fr))] gap-4">
          {results.map((card) => (
            <a
              key={card.href}
              href={card.href}
              className="flex items-stretch gap-4 rounded-lg bg-surface p-4 text-ink no-underline"
            >
              {card.image ? (
                <img
                  src={card.image.src}
                  ...
                  loading="lazy"
                  className="h-[170px] w-[150px] flex-none rounded-lg object-cover"
                />
              ) : (
                <div className="grid h-[170px] w-[150px] flex-none place-content-center rounded-lg bg-accent-2-100 p-3 text-center">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-2">
                ...
                <div className="mt-auto flex flex-wrap items-center gap-2">
                  {card.origin && (
                    <span className="rounded-lg bg-accent-2 px-3 py-1 text-[11.5px] font-semibold text-ink">
                      {card.origin}
                    </span>
                  )}
                  <span className="rounded-lg border-[1.5px] border-ink px-3 py-1 text-[11.5px] font-semibold text-ink">
